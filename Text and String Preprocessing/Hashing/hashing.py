class Hash:
    def __init__(self, s=None, vec=None, Base=0):
        ...
        self.p1, self.p2 = 37, 9973
        self.m1, self.m2 = 10**9 + 7, 10**9 + 9
        self.pow1, self.pow2, self.h1, self.h2 = [0] * (self.n + 5), [0] * (self.n + 5), [0] * (self.n + 5), [0] * (self.n + 5)
        ...
        if s is not None:
            ...
            for i in range(1, self.n + 1):
                self.h1[i] = (self.h1[i - 1] * self.p1 + ord(s[i - (not Base)])) % self.m1
                self.h2[i] = (self.h2[i - 1] * self.p2 + ord(s[i - (not Base)])) % self.m2
...
if  (hash_a.sub(1, 5) == hash_b.sub(6, 10)) :
    print("Equal")
else:    
    print("Not Equal")
